...
const calculateTpPrices = (dcaPrice: number, numOrders: number, minTpPercentage: number, maxTpPercentage: number) => {
    const tpPrices = [];
    const step = (maxTpPercentage - minTpPercentage) / (numOrders - 1);
    for (let i = 0; i < numOrders; i++) {
        const tpPercentage = minTpPercentage + i * step;
        tpPrices.push(dcaPrice * (1 + tpPercentage / 100));
    }
    return tpPrices;
};
...
const TradingCalc = () => {
    ...
    useEffect(() => {
        ...
        const tp = calculateTpPrices(dca, numOrders, minTpPercentage, maxTpPercentage);
        setTpPrices(tp);
        setAverageTp(calculateAverageTp(tp));
        setPercentageDiffs(calculatePercentageDiffs(prices, dca));
    }, [upperPrice, lowerPrice, numOrders, slPercentage, minTpPercentage, maxTpPercentage, averageTp]);
                ...
                <VStack spacing={4} width="60%" align="start">
                    ...
                    {entryPrices.map((price, index) => (
                        <Box key={index} p={2} borderWidth="1px" borderRadius="lg" width="100%">
                            ...
                            <Text>Entry Price: {price.toFixed(DECIMALS)}</Text>
                            <Text>Deviation from DCA: {percentageDiffs[index].toFixed(DECIMALS_PERCENTS)}%</Text>
